/// @file rollingpolicy_type_complete.c
/// @brief "complete" log4c rolling file policy implementation
/// @author MEP
...
/* Stores the length of last log to detect if file rollover is necessary 
 */
static int last_len = 0;

/* Stores the size of 0th file which is reopened to calculate when to rollover to next file
 */
static int val_to_add = 0;


/* Internal structs that defines the conf and the state info
 * for an instance of the appender_type_rollingfile type.
 */    
struct __complete_conf {  
  long swc_file_maxsize;
  long swc_file_max_num_files;
};

struct __complete_udata {
  struct __complete_conf sw_conf;
...
static int complete_init(log4c_rollingpolicy_t *this, rollingfile_udata_t* rfup);
static int complete_rollover(log4c_rollingpolicy_t *this, FILE **current_fpp );
static int complete_is_triggering_event(
			       log4c_rollingpolicy_t *this,
			       const log4c_logging_event_t* a_event,
			       long current_file_size);

static char **complete_make_filename_array(rollingpolicy_complete_udata_t *swup);
static int complete_get_last_index(rollingpolicy_complete_udata_t * swup);
static char* complete_get_filename_by_index(rollingpolicy_complete_udata_t * swup,
					   long i);
static int complete_open_zero_file(char *filename, FILE **fpp );
...
static int complete_is_triggering_event(
  ...
  long len = 0;
  int decision = 0;
  rollingpolicy_complete_udata_t *swup = log4c_rollingpolicy_get_udata(this);
  /*rollingfile_udata_t *rfup = log4c_rollingpolicy_get_udata(this);  */

  sd_debug("complete_is_triggering_event[");
  ...
  */
  ...
  if ( swup->sw_conf.swc_file_maxsize > 0 &&
      len + current_file_size + val_to_add > swup->sw_conf.swc_file_maxsize ) {
...
static int complete_rollover(log4c_rollingpolicy_t *this, FILE ** current_fpp ){
  ...
  */
  ...
  if ( !swup || !swup->sw_logdir){
   ...
   //printf("\n******* Value of k : %d \n", k);
   //printf("******* Max size is : %d \n", swup->sw_conf.swc_file_maxsize);

    struct stat st;
    stat(swup->sw_filenames[0], &st);
   ...
   if ( k < 0 ) {
     ...
     if (complete_open_zero_file(swup->sw_filenames[0], current_fpp)){
   ...
   } else if((st.st_size + last_len) < swup->sw_conf.swc_file_maxsize){

    val_to_add = st.st_size;
     if (complete_open_zero_file(swup->sw_filenames[0], current_fpp)){
       swup->sw_flags |= SW_LAST_FOPEN_FAILED;
       sd_error("open zero file failed");
     } else{
...
static int complete_init(log4c_rollingpolicy_t *this, rollingfile_udata_t *rfup){
  rollingpolicy_complete_udata_t *swup =  NULL;
  int i = 0;


  sd_debug("complete_init[");
  if (!this){
    goto complete_init_exit;
  }
  
  swup = log4c_rollingpolicy_get_udata(this);
  if ( swup == NULL ){
    swup = complete_make_udata();
  ...
  swup->sw_filenames = complete_make_filename_array(swup);
  for ( i = 0; i<swup->sw_conf.swc_file_max_num_files; i++){
    sd_debug("%s", swup->sw_filenames[i]);
  }
  swup->sw_last_index = complete_get_last_index(swup);  
...
static int complete_fini(log4c_rollingpolicy_t *this){
  rollingpolicy_complete_udata_t *swup =  NULL;
  int i = 0;
  int rc = 0;

  sd_debug("complete_fini[ ");
  if (!this){
    goto complete_fini_exit;
  }
  ...
  */
  sd_debug("freeing complete udata from rollingpolicy instance");
  free(swup);
...
/*******************************************************************************
                           complete specific conf functions
*******************************************************************************/

rollingpolicy_complete_udata_t *complete_make_udata(void){
  rollingpolicy_complete_udata_t *swup = NULL;
  swup = (rollingpolicy_complete_udata_t *)sd_calloc(1, 
                              sizeof(rollingpolicy_complete_udata_t));
  complete_udata_set_file_maxsize(swup,
				  ROLLINGPOLICY_SIZE_DEFAULT_MAX_FILE_SIZE);
  complete_udata_set_max_num_files(swup,
...
int complete_udata_set_file_maxsize(rollingpolicy_complete_udata_t * swup,
					 long max_size){

  if (max_size <= 0)
  	swup->sw_conf.swc_file_maxsize = ROLLINGPOLICY_SIZE_DEFAULT_MAX_FILE_SIZE;
  else
...
int complete_udata_set_max_num_files(rollingpolicy_complete_udata_t *swup, 
					  long max_num){
...
int complete_udata_set_rfudata(rollingpolicy_complete_udata_t *swup,
...
static char **complete_make_filename_array(rollingpolicy_complete_udata_t *swup){
  ...
  while ( i <  swup->sw_conf.swc_file_max_num_files){
    ...
    filenames[i] = complete_get_filename_by_index(swup,i);
    i++;
  }
  return(filenames);
}
...
static int complete_open_zero_file(char *filename, FILE **fpp ){
  int rc = 0;
  sd_debug("complete_open_zero_file['%s'", filename);
...
int delete_log_files(rollingpolicy_complete_udata_t *swup)
{
  ...
  int count_files = complete_get_last_index(swup) + 1;

  for ( i = 0; i<count_files; i++){
    //printf(" files : %s\n", swup->sw_filenames[i]);
    remove(swup->sw_filenames[i]);
  }
